    ...
    for (Long64_t i = 0; i < tree->GetEntries()/10; i++) {
        ...
        if(!useGEMs){
            ...
            for (int j = 0; j < ev.matchNum; j++) {
                ...
                if(theta > 1.2) veto = false;

                if (isMott(E, Ebeam, resolution) && !veto) {
...
void q2_plot(const char *files = "../data/0.7GeV/prad_024640_filter.root",
    ...
    TH1F *acceptance_q2 = nullptr;
    {
        ...
        {   
            ...
            if (!gf || gf->IsZombie()) {
                std::cerr << "ERROR: cannot open 0.7GeV_e-.root" << std::endl;
                return;
            }
