...
double GetArealDensity(double pressure_mTorr) {
    ...
    const double T   = 20.;         // cell temperature [K]
    ...
    for (Long64_t i = 0; i < tree->GetEntries(); i++) {
        ...
        if(!useGEMs){
            for (int j = 0; j < ev.n_clusters; j++) {
                ...
                float y = ev.cl_y[j];
                float z = ev.cl_z[j];
                float E = ev.cl_energy[j]*1.062;
                float theta = atan2(std::sqrt(x*x + y*y), z) * 180.f / M_PI;

                if( fabs(x) < 20.75 * 2.1 && fabs(y) < 20.75 * 2.1 ) continue;
            ...
            if (ev.n_clusters == 2){
                float x1 = ev.cl_x[0], y1 = ev.cl_y[0], z1 = ev.cl_z[0], E1 = ev.cl_energy[0]*1.062;
                float x2 = ev.cl_x[1], y2 = ev.cl_y[1], z2 = ev.cl_z[1], E2 = ev.cl_energy[1]*1.062;
            ...
            for (int j = 0; j < ev.matchNum; j++) {
                ...
                float E = ev.mHit_E[j]*1.062;
                float theta = atan2(std::sqrt(hit.x*hit.x + hit.y*hit.y), hit.z) * 180.f / M_PI;

                if( fabs(hit.x) < 20.75 * 2.1 && fabs(hit.y) < 20.75 * 2.1 ) continue;
...
void q2_plot(const char *files = "../data/0.7GeV/prad_024640_filter.root",
    ...
    double density = GetArealDensity(656.); // atoms/cm^2 per mTorr
    ...
    std::vector<TString> fileListB = {"../data/0.7GeV/prad_024639_filter.root"};
    ...
    {
        const double L_gen_pb  = 9.7003e-03;       // generator integrated luminosity [pb^-1]
        ...
        {   
            ...
            TFile *gf = TFile::Open("../data/0.7GeV/0.7GeV_e-.root");
            ...
            for (Long64_t i = 0; i < nt->GetEntries(); i++) {
                ...
                if( (fabs(x) > 20.75 * 2.1 || fabs(y) > 20.75 * 2.1)
                    && (fabs(x) < 20.75 * 16. && fabs(y) < 20.75 * 16.) && theta > 0.7f)
    ...
    TH1F *double_arm_geo_acc  = new TH1F("double_arm_geo_acc",  "Moller Double-Arm Geometric Acceptance;Q^{2} (GeV^{2});Acceptance (%)", Nbins, q2_edges);
    {
        ...
        auto inHyCal = [](double xmm, double ymm) {
            return (fabs(xmm) > 20.75 * 2.1 || fabs(ymm) > 20.75 * 2.1)
    ...
    cross_section->SetTitle("e-p Cross Section @ 0.7GeV (corrected for acceptance)");
