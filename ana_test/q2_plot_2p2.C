...
float gem_efficiency[Nbins] = {85.3744, 90.7219, 92.0117, 91.3683, 92.2211, 93.3969, 
    92.9094, 92.6417, 92.4142, 91.5083, 90.5655, 90.5462, 90.7062, 89.352, 88.3541, 
    88.605, 87.8988, 86.9411, 85.3229, 86.0954, 87.2335, 86.8753, 82.3255, 79.5598, 
    83.442, 85.5758, 86.1305, 85.6542, 83.6575, 81.7351, 82.913, 84.9415, 81.8445};
...
void q2_plot_2p2(const char *files = "../data/2.2GeV/prad_025218_filter.root",
                double lc = 248315){
    ...
    std::vector<TString> fileListB = {"../data/2.2GeV/prad_025225_filter.root"};
    ...
    {
        const double L_gen_pb  = 9.0587e-03;       // generator integrated luminosity [pb^-1]
        ...
        {   
            ...
            TFile *gf = TFile::Open("../data/2.2GeV/2.2GeV_e-.root");
            if (!gf || gf->IsZombie()) {
                std::cerr << "ERROR: cannot open 2.2GeV_e-.root" << std::endl;
                return;
            }
            TNtuple *nt = (TNtuple*)gf->Get("ntp");
            if (!nt) {
                std::cerr << "ERROR: ntuple 'ntp' not found in 2.2GeV_e-.root" << std::endl;
                gf->Close(); return;
            }
    ...
    for (int i = 1; i <= mott_yield->GetNbinsX(); i++) {
        ...
        gem_efficiency[i-1] *= 0.01;  // convert percentage to fraction
